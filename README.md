...
1. 1차 캐시
    ...
    ```java
       ...
       System.out.println(member1 == member2) // 동일성 보장 true
    ```
1. 쓰기 지연
...
1. 지연로딩
    ...
    - 연관 객체는 어플리케이션에서 실제 사용될 때 DB로 부터 조회된다.(DB에 조회 쿼리가 질의된다.)



### 영속성 컨택스트

- 영속 상태의 엔티티를 가지고 있는 컨택스트
...
1. 1차 캐시
   ...
   - 영속성 컨택스는 key(pk) & value(객체)의 형태로 엔티티를 관리한다. 관리되고 있는 엔티티의 상태를 영속 상태라고 한다.
   - 조회시 영속성 컨택스트에 이미 영속되어 있는 엔티티가 있다면 캐시된 엔티티를 즉시 반환하고 없다면 DB에서 조회 후 영속성 컨택스트에 저장한다.
   - 단, 영속성 컨택스트는 트랜잭션 시작시 생성되었다가 종료시 삭제되므로 1차 캐시라 한다. (순간적으로 만들어지고 사라짐.)

2. 동일성 보장

   - 영속된 엔티티는 동일한 엔티티라면 동일성을 보장 해주므로 == 비교시 true를 반환한다.

3. 트랜잭션을 지원하는 쓰기 지연

   - `em.persist(member)`는 즉시 insert 쿼리를 DB에 질의하지 않고 영속성 컨택스트 내부에 존재하는 쓰기 지연 저장소에 저장 해뒀다가 트랜잭션 커밋하는 시점에 DB에 질의된다.(flush)
   - batch insert와 같은 성능상 이점이 있도록 코딩할 수 있다.

4. 변경 감지 (Dirty Check)

   - 영속 상태의 엔티티를 변경하면 영속성 컨택스트는 이를 캐치하여 쓰기 지연 SQL 저장소에 update 쿼리를 생성하고 flush(트랜잭션 커밋) 시점에 DB로 질의한다.

   
### 플러시

영속성 컨택스트의 쓰기 지연 저장소의 내용을 데이터베이스로 전송한다. (플러시 해도 영속 상태의 객체들은 영속 상태로 유지된다.)
...
1. 트랜잭션 커밋시 자동 호출
1. JPQL 쿼리 실행시 자동 호출
    - JPQL은 SQL로 변경되어 영속성 컨택스트와 아무런 상관이 없이 DB에 질의 된다. 이에따른 오동작을 방지하기 위해서 기본으로 플러시 된 후SQL이 실행된다.
    

##### 플러시 발생시 영속성 컨택스트의 동작

1. 변경 감지 (영속 상태의 엔티티 중 변경된 엔티티가 있다면)
1. 쓰기 지연 SQL 저장소의 쿼리를 데이터베이스에 전송(질의)

##### 플러시 모드 옵션

`em.setFlushMode(FlushModeType.COMMIT)`

1. `FlushModeType.AUTO` : 커밋이나 쿼리를 실행할 때 플러시 (기본값, 특별한일 없으면 기본 설정으로 사용)
...
1. 필드와 컬럼 매핑
   ...
   ```
   - `@Column`의 속성
       - name : 필드와 매핑할 컬럼명 지정 (기본 값 : 객체의 필드명)
       ...
       - unique : 유니크 제약 조건을 부여 - 하지만 제약명이 임의로 생성되므로 @Table 애노테이션에서 제약을 추가하는 것을 선호
       
       - columnDefinition : 컬럼에 정보를 부여 (DDL에 문구가 그대로 추가되므로 특정 DB에 종속적인 제약 조건을 걸 수 있다.)
       - length : String 타입에서만 사용가능한 길이 제한
       - precision : BigDecimal 타입에서만 사용할 수 있는 소수점 자릿수 제한
       
   - `@Enumerated` 사용시 주의 사항
       
    - value : 기본값이 EnumType.ORDINAL(enum의 순서)다. 순서는 변경이 될 수 있으므로 EnumType.STRING을 사용하는 것을 강력 권장 
   - `@Temporal` 사용시 주의 사항
...
1. 키본 키 매핑
    ...
    - `@GeneratedValue` : 자동 지정
        - IDENTITY : 데이터베이스에 위임, IDENTITY 전략에서는 쓰기 지연을 제공하지 않는다. 이유는 DB에 Insert 해봐야 값을 알 수 있으므로 (PK를 알아올 수 있도록)
        ...
        ```
            - allocationSize = 50 (기본 값) : 시퀀스 값을 50씩 증가시킨다. (성능 최적화에 사용) 시퀀스를 미리 생성해두고(미리 확보해두고) 메모리에서 하나식 꺼내서 사용한다.
      
        - TABLE : 키 생성용 테이블 사용, @TableGenerator 사용 (거의 사용하지 않음)
...
1. 연관관계 매핑
    ...
    - 방향 (단방향, 양방향)
        1. 단방향 연관관계
            ...
            - Member에서 Team으로는 연관관계가 설정되어 있지만 반대로는 설정되어 있지 않음.
        ...
        1. 양방향 연관관계
            ...
            ```
            - 연관관계 주인과 mappedBy
                - 객체는 단방향 + 단방향이 합쳐서 양방향으로 작동하지만 테이블은 하나의 외래키로 양방향으로 작동(조인)이 가능하다.
                - 외래키를 양쪽에서 관리할 수 없으므로 객체의 두 관계 중 하나를 연관 관계의 주인으로 지정하고 **주인만이 외래키를 관리(등록, 수정)**
                  주인이 아닌쪽은 읽기만 가능하다. (즉 `team.getMembers().add(member3)` 해도 아무 소용 없다. DB에 질의되지는 않지만 객체 지향적으로 봤을때는 필요할 수 있는 코드)
                - **주인은 mappedBy 속성을 사용하면 안된다.** 주인이 아닌 객체만 mappedBy 속성으로 주인 지정
            - **단방향 매핑만으로도 이미 연관관계 매핑은 완료**되지만, 반대 방향에서 탐색하는 기능을 부여하기 위해서 양방향 매핑을한다. 하지만 양방향 매핑에서 발생하는 이슈가 몆몆 있으므로 단방향 매핑 위주로 하되 필요할 때 (비즈니스 코드에서 자주 반대 방향의 데이터를 사용해야 될 때) 양방향 매핑을 추가하는 것 이 바람직하다.
    - 다중성 : 다대일, 일대다, 일대일, 다대다
        1. 다대일
            - 연관 관계가 N:1인 관계 (선수들은 하나의 팀에만 속해 있다.)
            - **연관 관계의 주인이 N인 객체**가 되는 관계
        2. 일대다
            - 연관 관계가 1:N인 관계 (하나의 팀에 다수의 선수들이 속해 있다.)
            - **연관 관계의 주인이 1인 객체**가 되는 관계
            - 권장 하지는 않는 모델 : 테이블 구조에서 결국 외래키는 N인 테이블에 존재하므로 연관 관계의 주인인 객체와 외래키가 있는 테이블의 불일치로 객체와 관련 없는 테이블의 키를 넣는 추가 쿼리 발생
        3. 일대일
            - 연관 관계가 1:1인 관계 (한명의 회원은 하나의 락커를 가질수 있다.)
            - 연관 관계의 주인은 회원도 가능, 락커도 가능
            - 매핑 방법은 다대일 연관 관계와 비슷
    4. 다대다
            - 매핑 테이블이 자동으로 생성되어 관리된다. 하지만 실무에서는 단순하게 연결 정보만 가지고 있는 테이블은 거의 없다. (등록시간, 등록ID 같은 정보들이 대부분 들어감.) 그러므로 실무에서는 매핑 테이블 역할을 하는 엔티티를 중간에 추가해서 일대다 - 다대일 관계를 가지는 것을 권장한다.
...
1. 상속 관계 매핑
    ...
    - @Inheritance
        - InheritanceType.SINGLE_TABLE (기본 전략) : 하나의 테이블(Item)에 모든 값(Item + Album + Book)이 들어감
            ...
            - 자손테이블의 정보를 모두 가지고 있으므로 테이블의 사이즈가 커지고 타입에 따라 불필요한 컬럼은 null을 허용 해야하므로 nullable 컬럼이 많아진다.
...
- `em.getReference()` : 데이터베이스 조회를 미루는 가짜 객체(프록시)가 반환된다.

#### 프록시 특징

- 프록시 객체는 멤버 변수로 실제 객체를 참조하고 있다. 프록시를 호출하면 값의 존재 여부를 확인하고 값이  없다면 영속성 컨택스트에 초기화를 요청 하여 반환된 실제 객체의 메서드 호출
- 프록시 객체는 한 번만 초기화 (초기화된 실제 객체는 즉 프록시가 초기화 된다고 실제 객체가 되는것은 아니다.)
...
- 모든 엔티티에 즉시 로딩을 질의하면 예상하지 못한 SQL이 발생할 수 있다.
...
- 값 타입은 크게 엔티티 타입, 값 타입으로 나뉠수 있다.
- 엔티티 타입 : 식별자(PK)가 있다. 생명 주기가 관리된다. 공유할 수 있다.
- 값 타입 : 식별자가 없다. 생명 주기를 엔티티에 의존한다. 공유할 수 있지만 하지 않는것이 안전하므로 설계상 장치를 마련해야 한다.(불변 객체로 만들자.)
...
- 엔티티가 아닌 그냥 값 타입, 주로 기본 값 타입을 모아서 만드는 복합 값 형태이다
...
- 데이터베이스는 컬렉션을 같은 테이블에 저장할 수 없으므로 **컬렉션을 저장하기 위한 별도의 테이블이 필요하므로 테이블을 만든다.**
...
- SQL을 추상화한 객체지향쿼리
- JPA를 사용하면 엔티티 객체를 중심으로 개발한다. 조회 쿼리와 같은 쿼리도 테이블의 데이터가 아닌 엔티티 객체를 대상으로 검색해야 하지만 모든 데이터를 객체로 변환해서 검색하는 것은 불가능하므로 결국 검색 조건이 포함된 SQL이 필요
```java
List<Member> members = 
            em.createQuery(
